...
import { fetchManifest, addDotToManifest, saveManifest, uploadDotImage, imageUrlForDot } from "./lib/cloudinary";
...
export const Dots = forwardRef(({ count = 1000000, radius = 6.2, dotRadius = 2.2 }, ref) => {
	...
	useEffect(() => {
		...
		(async () => {
			...
			for (const dotId of manifest.dots) {
				...
				if (!pos) continue;
				next[dotId] = { ...pos, texture: loader.load(imageUrlForDot(dotId), () => invalidate()) };
			}
	...
	const handleDotClick = (eventOrIndex) => {
		...
		fileInput.onchange = async (e) => {
			...
			try {
				// Upload to Cloudinary with deterministic name
				await uploadDotImage(dotIndex, file);
				const imageUrl = imageUrlForDot(dotIndex);
				...
				if (dotPosition) {
					...
					// Update manifest in Cloudinary raw file
